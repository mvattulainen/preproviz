...
vignette: >
  %\VignetteIndexEntry{preproviz package: Tools for Visualization of Interdependent Data Quality Issues for Faster
...
Package 'preproviz' supports preprocessing decision making in the following topics:  
...
3. Are there data quality clusters (i.e. groups of data points whose data quality
characteristics are similar to each other)? Method: PlotCMDS  
...
Method: plotOUTLIERS? Do outlier scores in the original data match with the outlier  
scores in the constructed data? Method: plotLOFSUM  

6. Do constructed data quality features have predictive power? plotVARIMP  
...
MissingValueToClass: Number of missing value in a row divided by average number of missing value in rows of the same class
...
Scatter Counter: Scatter counter score in 1/10 neighborhood (Scatter Counter algorithm by Siermala and Juhola)
...
3. Feature construction: xxample: number of missing values (counted before missing value imputation)
...
ControlClass, argument for preproviz() function references by name to one or several SetUpClass objects.  
...
Data must be in a data frame that has one and only one factor column for class labels.
Other columns must be numeric. The factor column can not contain missing values.

There must be three or more constructed features in the ParameterClass object.

Constructed features that have constant value are not processed (e.g. MissingValueShare  
if there are no missing values in the data set).  
